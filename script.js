...
(function () {
  ...
  const COUNT = 110;
  const ATTRACT_RADIUS = 520 * 1; // alcance de la atracción (en px, se escala por dpr)
  const ATTRACT_FORCE = 0.03;     // fuerza con la que se juntan hacia el ratón
  const AFTERGLOW = 90;           // frames que siguen atraídas tras apartar el ratón
  ...
  window.addEventListener("mouseleave", () => {
    mouse.active = false; // el afterglow (mouse.glow) sigue bajando solo
  });
  ...
  function draw() {
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      // atracción hacia el ratón: las figuritas se juntan al cursor
      // (mouse.active) mientras hay ratón, y (mouse.glow>0) un rato después
      ...
      p.x += p.vx; p.y += p.vy;
      // fricción suave para que no se disparen
      p.vx *= 0.985; p.vy *= 0.985;
      // deriva mínima para que sigan vivas
